...
export const EventForm = ({ initialData, onSubmit, onCancel, isLoading }: EventFormProps) => {
  ...
  const handleSwitchChange = (name: string, checked: boolean) => {
    if (checked) {
      setFormData(prev => ({
        ...prev,
        [name]: checked,
        price: { inPerson: { amount: 0, currency: 'NGN' }, virtual: { amount: 0, currency: 'NGN' } },
      }));
    } else {
      setFormData(prev => ({
        ...prev,
  ...
  const handlePriceChange = (type: 'inPerson' | 'virtual', field: 'amount' | 'currency', value: string) => {
    setFormData(prev => ({
      ...prev,
      price: {
        ...prev.price,
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.isPaid) {
      if (!formData.price?.inPerson?.amount && !formData.price?.virtual?.amount) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="title" name="title" value={formData.title} onChange={handleInputChange} placeholder="Enter event title" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="link" name="link" value={formData.link} onChange={handleInputChange} placeholder="https://example.com/event" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              />
              {errors.location && <p className="text-sm text-red-500">{errors.duration}</p>}
            </div>

            {(formData.eventType === 'inPerson' || formData.eventType === 'virtual' || formData.eventType === 'hybrid') && (
              <div className="space-y-2">
                <Label htmlFor="location">{formData.eventType === 'virtual' ? 'Platform *' : 'Location *'}</Label>
                <Input
                  id="location"
                  name="location"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedImages.map((file, index) => (
                        <div key={`new-${index}`} className="relative">
                          <img src={URL.createObjectURL(file)} alt={`New ${index + 1}`} className="w-20 h-20 object-cover rounded" />
                          <button
                            type="button"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={e => handlePriceChange('inPerson', 'amount', e.target.value)}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={e => handlePriceChange('inPerson', 'currency', e.target.value)}
                      />
                    </div>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={e => handlePriceChange('virtual', 'amount', e.target.value)}
                      />
                    </div>
                    <div className="space-y-2">
                      <Label>Currency</Label>
                      <Input
                        ...
                        onChange={e => handlePriceChange('virtual', 'currency', e.target.value)}
                      />
                    </div>
                  </div>
                )}
